...
input { width:100%; padding:12px; margin-bottom:12px; border-radius:10px; border:none; outline:none; background:#111122; color:white; font-size:14px; border:1px solid rgba(128,0,255,0.3); }
input:focus { border:1px solid #bb86fc; box-shadow:0 0 10px #bb86fc; }
...
<button id="predictBtn">Predict Semester 3 Score</button>
<h2 id="result"></h2>
...
function login() {
    ...
    if(!username||!password){ document.getElementById("loginStatus").innerText="Please enter username and password."; return; }
...
async function trainModel(){
    ...
    if(currentUser){ const users=JSON.parse(localStorage.getItem("students"))||{}; const history=users[currentUser].history||[]; history.forEach(item=>{ xsData.push([item.lastYear,item.s1,item.s2,item.attendance,item.study,item.homework,item.screenTime]); ysData.push([item.predicted]); }); }
...
async function predictScore(){
    ...
    document.getElementById("result").innerText="Predicted Semester 3 Score: "+score.toFixed(2);
    ...
    if(currentUser){ let users=JSON.parse(localStorage.getItem("students"))||{}; users[currentUser].history.push({lastYear,s1,s2,attendance,study,homework,screenTime,predicted:score}); localStorage.setItem("students",JSON.stringify(users)); }
}
